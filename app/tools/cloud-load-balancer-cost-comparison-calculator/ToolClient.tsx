          ...
          <div className="mt-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <button
                type="button"
                onClick={resetRates}
                className="rounded-full border border-[#165A31] px-3 py-1.5 text-xs font-semibold text-[#165A31] hover:bg-[#f4fbf6]"
              >
                Reset rates
              </button>
            </div>
            ...
            <div className="mt-4 space-y-2">
              {results.map((result, index) => (
                <button
                  ...
                  className={`grid w-full min-w-0 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 rounded-lg border px-3 py-3 text-left transition ${
                    result.provider.id === activeProvider.id
                      ? "border-[#165A31] bg-[#f4fbf6]"
                      : "border-slate-200 bg-white hover:border-[#165A31]"
                  }`}
                >
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#165A31] text-xs font-semibold text-white">
                    {index + 1}
                  </span>
