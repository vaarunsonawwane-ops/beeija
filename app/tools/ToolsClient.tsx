...
import { BEEIJA_CATEGORIES } from "@/app/data/tools";
...
export default function ToolsClient({ tools }: ToolsClientProps) {
  ...
  const categories = useMemo(() => {
    ...
    return foundCategories.sort((a, b) => {
      const aIndex = preferredCategoryOrder.indexOf(
        a as (typeof BEEIJA_CATEGORIES)[number],
      );
      const bIndex = preferredCategoryOrder.indexOf(
        b as (typeof BEEIJA_CATEGORIES)[number],
      );
              ...
              return (
                <label
                  ...
                  className={`flex cursor-pointer items-center gap-2 rounded-xl border px-4 py-2 text-sm transition ${
                    checked
                      ? "border-[var(--green)] bg-[rgba(22,90,49,0.08)] text-[var(--dark)]"
