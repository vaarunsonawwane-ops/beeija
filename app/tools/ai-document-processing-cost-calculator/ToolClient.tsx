      ...
      <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm md:p-8">
        ...
        <div className="mt-7 rounded-xl border-l-4 border-[#F2C94C] bg-[#F5FAF7] px-5 py-4">
          ...
          <div className="mt-3 grid gap-2 text-sm text-gray-700 sm:grid-cols-2">
            <p>
              Base pages: {formatInteger(result.basePages)}
            </p>

            <p>
              Processed pages after retries:{" "}
              {formatInteger(result.processedPages)}
            </p>

            <p>
              Image-heavy pages:{" "}
              {formatInteger(result.imageHeavyPages)}
            </p>

            <p>
              LLM-validated documents:{" "}
              {formatInteger(result.llmDocuments)}
            </p>

            <p>
              Human-reviewed documents:{" "}
              {formatInteger(result.reviewedDocuments)}
            </p>

            <p>
              Successful documents:{" "}
              {formatInteger(result.successfulDocuments)}
            </p>

            <p>
              Failed or incomplete documents:{" "}
              {formatInteger(result.failedDocuments)}
            </p>

            <p>
              Manual-only labour hours:{" "}
              {formatNumber(result.manualHours)}
            </p>
          </div>
        </div>
        ...
        stats={
          <div className="grid gap-4 sm:grid-cols-3">
            <ResultStat
              ...
              value={
                result.hasAnyPrice
                  ? formatMoney(result.costPerDocument)
                  : "—"
              }
            />
            ...
            <ResultStat
              ...
              value={
                result.hasAnyPrice
                  ? formatMoney(result.costPerPage)
                  : "—"
              }
            />
            ...
            <ResultStat
              ...
              value={
                result.hasAnyPrice
                  ? formatMoney(result.costPerSuccessfulDocument)
                  : "—"
              }
            />
          </div>
